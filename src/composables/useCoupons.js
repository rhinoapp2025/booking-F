...
import { useHotelRoute } from './useHotelRoute'
...
export function useCoupons() {
  ...
  async function loadCouponSettings() {
    try {
      const { data } = await api.get(`/api/coupons/${hotelSlug.value}/settings`)
  ...
  async function loadMyCoupons() {
    try {
      const { data } = await api.get(`/api/coupons/${hotelSlug.value}/my`)
  ...
  async function redeemCoupon() {
    ...
    const result = await Swal.fire({
      ...
      text: `ใช้ ${requiredPoints.toLocaleString('th-TH')} แต้มของโรงแรมนี้เพื่อแลกคูปอง 1 ใบ ใช่หรือไม่?`,
      icon: 'question',
    ...
    try {
      const { data } = await api.post(`/api/coupons/${hotelSlug.value}/redeem`)
      await auth.fetchMe()
      await loadMyCoupons()
