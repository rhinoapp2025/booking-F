...
function chatImageRequestUrl(filename) {
  ...
  const hotelSlug = localStorage.getItem('hotelSlug') || 'default'
  return `${baseURL}/api/chat/${encodeURIComponent(hotelSlug)}/images/${encodeURIComponent(filename)}`
}

function chatImageRequestHeaders() {
  const headers = {}
  const token = localStorage.getItem('token')
  const shopSlug = localStorage.getItem('hotelSlug')
  if (token) headers.Authorization = `Bearer ${token}`
  if (shopSlug) headers['X-Hotel-Slug'] = shopSlug
  return headers
}
