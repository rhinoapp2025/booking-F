...
const { hotelPath: shopPath } = useHotelRoute()

const menuOpen = ref(false)

const isPhoneAccount = computed(() => auth.user?.provider === 'phone' || Boolean(auth.user?.login_id))

const loginLabel = computed(() => {
  if (auth.user?.login_id) return auth.user.login_id
...
const userSubtitle = computed(() => loginLabel.value)
